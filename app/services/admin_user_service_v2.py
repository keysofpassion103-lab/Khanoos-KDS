...
class AdminUserServiceV2:
    ...
    async def register(data: AdminUserCreate) -> Dict:
        ...
        try:
            ...
            existing = supabase.table("admin_users").select("id, auth_user_id").eq(
                "email", data.email
            ).execute()
    ...
    @staticmethod
    async def get_profile(auth_user_id: str) -> Dict:
        """Get admin user profile by Supabase Auth user ID"""
    ...
    @staticmethod
    async def update(auth_user_id: str, data: AdminUserUpdate) -> Dict:
    ...
    @staticmethod
    async def change_password(auth_user_id: str, new_password: str) -> Dict:
